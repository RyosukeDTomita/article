...
```
これで日本語化は完了なので再起動する。
> 日本語になっていない時はさらに再度再起動してみると良い。

再起動すると、ディレクトリ名を日本語に変更するか聞かれるが特に希望がなければ古いままにするを選択するのをおすすめする。

### キーボードの設定の変更
- 設定マネージャーを開いて、キーボード→レイアウト→システムデフォルトを使用するのチェック外す。
- 次にレイアウトオプションの変更で一番上の-を選択する。
- キーボードレイアウト日本語OADG109Aを選択する。
- 英語(US)キーボードが残っていれば削除する。
